import os
import MDAnalysis
from MDAnalysis.newanalysis.helpers import calcEnergyAtomicVoro
...
epn=np.zeros((tlen,mqs1.numberOfAtoms()))

for j in range(tlen):
    for i in range(mqs1.numberOfAtoms()):
        epn[j,i]=epa[j,i,:].sum()
